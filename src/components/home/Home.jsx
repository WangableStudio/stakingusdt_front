...
function Body() {
    ...
    const [time, setTime] = useState(new Date().toLocaleTimeString("en-GB", { timeZone: "Asia/Yekaterinburg" }));
    ...
    useEffect(() => {
        const fetchRates = async () => {
            try {
                ...
                setUsdtRate(data.tether.usd);
                // Примерное значение процента, можно заменить на данные из API, если доступны
                setPercentChange(4.19);
            } catch (error) {
        ...
        const updateTime = () => {
            setTime(new Date().toLocaleTimeString("en-GB", { timeZone: "Asia/Yekaterinburg" }));
        };

        fetchRates();
        updateTime();

        const intervalRates = setInterval(fetchRates, 60000); // Обновлять курсы каждую минуту
        const intervalTime = setInterval(updateTime, 1000);   // Обновлять время каждую секунду
                        ...
                        <div className="main_promo">
                            <div className='infoPromo'>
                                <div className='text_info_block'>
                                    <p className='timePromo'>USDT {time} GMT+5</p>
                                    <p className='coursePromo'>{usdtRate ? `${usdtRate.toLocaleString()} USDT` : "loading..."}</p>
                                    <p className='profitPromo'>{percentChange ? `+${percentChange}% сегодня` : "loading..."}</p>
